...
fun hauntedWasteland1(): String {
    ...
    class Path(val potentialEnds: List<Long>, val period: Long)

    class LoopInfo(
        val stepOffset: Long,
        val startingNode:
        String, val firstInstructionIndex: Int,
        val period: Long
    )

    fun calculatePath(startingNode: String): Path {
        fun findLoopStart(): LoopInfo {
            ...
            while (true) {
                ...
                if (visitedAtStep != null) {
                    println("Found loop: $startingNode")
                    return LoopInfo(stepCounter - 1, target, instructionIndex, )
                }
        ...
        fun findPotentialEnds(startingNode: String, firstInstructionIndex: Int): ArrayList<Long> {
        ...
        val loopStart = findLoopStart()
        val potentialEnds = findPotentialEnds(loopStart.startingNode, loopStart.firstInstructionIndex)
    }

    val starts = map.keys.filter { k -> k[2] == 'A' }
    val paths = runBlocking(Dispatchers.Default) {
        starts.map { s ->
            calculatePath(s)
        }
    }
